...
var ColorPickerGrid = A.Component.create(
	{
		...
		prototype: {
			...
			_uiSetColors: function(value) {
				...
				var buffer = [];
				var tplBuffer = [TPL_ITEM_OPEN, STR_BLANK, TPL_ITEM_CONTENT_OPEN, STR_BLANK, TPL_ITEM_CLOSE];
				...
				A.each(
					...
					function(item, index, collection) {
						...
						buffer[index] = tplBuffer.join(STR_BLANK);
					}
				);

				var pickerContainer = instance.get('host')._pickerContainer;

				pickerContainer.setContent(buffer.join(STR_BLANK));
			}
		}
	}
);
