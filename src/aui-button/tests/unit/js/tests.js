    ...
    suite.add(new Y.Test.Case({
        ...
        '#1: Focusing first input should position .btn-search-cancel on the right.': function() {
            ...
            inputNode.once('click', function(event) {
                setTimeout(function() {
                    test.resume(function() {
                        var xpos = event.target.get('region').right - event.target.next(
                            '.btn-search-cancel').get('region').width + searchButtonCancel.get(
                            'gutter')[0];

                        var ypos = event.target.get('region').top + event.target.get('region').height /
                            2 - event.target.next('.btn-search-cancel').get('region').height /
                            2 + searchButtonCancel.get('gutter')[1];

                        Y.Assert.areSame(event.target.next('.btn-search-cancel').getY(), ypos,
                            'Y-Position of input and search-btn-cancel should be the same.');

                        Y.Assert.areSame(event.target.next('.btn-search-cancel').getX(), xpos,
            ...
            setTimeout(function() {
                ...
                test._forceClickListener(inputNode);
            }, 0);
        ...
        '#2: Focusing second input should position .btn-search-cancel on the right.': function() {
            ...
            setTimeout(function() {
                ...
                test._forceClickListener(inputNode2);
            }, 0);
        ...
        '#3: .btn-search-cancel should be positioned correctly when input is moved by removing from dom and placing in a different area': function() {
            var searchBtnCancelDemo,
                searchBtnCancelDemoParent;

            searchBtnCancelDemo = Y.one('#search-btn-cancel-demo');
            ...
            searchBtnCancelDemoParent.one('.search-btn-cancel-heading').insert(searchBtnCancelDemo,
                'before');

            Y.each(Y.all('input.clearable'), function(input) {
                var xpos = input.get('region').right - input.next('.btn-search-cancel').get('region').width +
                    searchButtonCancel.get('gutter')[0];

                var ypos = input.get('region').top + input.get('region').height / 2 - input.next(
                    '.btn-search-cancel').get('region').height / 2 + searchButtonCancel.get('gutter')[1];

                Y.Assert.areSame(input.next('.btn-search-cancel').getY(), ypos,
                    'Y-Position of input and search-btn-cancel should be the same.');

                Y.Assert.areSame(input.next('.btn-search-cancel').getX(), xpos,
                    'X-Position of input and search-btn-cancel should be the same.');
            });
        },

        _forceClickListener: function(inputNode) {
            ...
            if (Y.UA.gecko) {
                searchButtonCancel._syncButtonUI(inputNode);
            }
        }
    }));
