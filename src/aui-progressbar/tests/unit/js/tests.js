YUI.add('aui-progressbar-tests', function(Y) {
    ...
    suite.add(new Y.Test.Case({
        ...
        'should render correct markup': function() {
            ...
            Y.Assert.isTrue(this.progressBar.get('boundingBox').hasClass('progress'), 'Rendered markup should have the \'progress\' CSS class');

            Y.Assert.isNotNull(contextBox.hasClass('progress-bar'), 'Rendered markup should have a child with the \'progress-bar\' CSS class');

            Y.Assert.areEqual(labelText, contextBox.get('text'), 'Rendered markup should have the specified label');
        },
        ...
        'should add ARIA attributes': function() {
            ...
            Y.Assert.areEqual(label, boundingBox.getAttribute('aria-label'), 'aria-label should be set to specified label');
            Y.Assert.areEqual(max, boundingBox.getAttribute('aria-valuemax'), 'aria-valuemax should be set to specified max');
            Y.Assert.areEqual(min, boundingBox.getAttribute('aria-valuemin'), 'aria-valuemin should be set to specified min');
            Y.Assert.areEqual(orientation, boundingBox.getAttribute('aria-orientation'), 'aria-orientation should be set to specified orientation');
            Y.Assert.areEqual(value, boundingBox.getAttribute('aria-valuenow'), 'aria-valuenow should be set to specified value');
        },
        ...
        'should not add ARIA attributes if requested': function() {
            ...
            Y.Assert.isFalse(boundingBox.hasAttribute('aria-orientation'), 'aria-orientation should not be set');
        ...
        'should correctly calculate ratio and step': function() {
            ...
            Y.Assert.areEqual(0.75, this.progressBar.get('ratio'), 'Calculated ratio should be updated to 0.75');
        ...
        'should respect orientation': function() {
            ...
            Y.Assert.areEqual('80px', contentBox.getStyle('width'), 'The width should be set appropriately on horizontal orientation');
            Y.Assert.areEqual('100%', contentBox.getStyle('height'), 'The height should be 100% on horizontal orientation');

            this.progressBar.set('orientation', 'vertical');

            Y.Assert.areEqual('100%', contentBox.getStyle('width'), 'The width should be 100% on vertical orientation');
            Y.Assert.areEqual('80px', contentBox.getStyle('height'), 'The height should be set appropriately on vertical orientation');
        },
