...
var L = A.Lang,
	...
	TPL_CALENDAR_DAY_BLANK = '<div class="' + [ CSS_DAY_BLANK, CSS_HIDE ].join(SPACE) + '"></div>',

	TPL_CALENDAR_DAY_PADDING_START = '<div class="' + [ CSS_DAY, CSS_STATE_DEFAULT, CSS_DAY_PADDING_START, CSS_HIDE ].join(SPACE) + '"></div>',

	TPL_CALENDAR_DAY_PADDING_END = ['<div class="' + [ CSS_DAY, CSS_STATE_DEFAULT, CSS_DAY_PADDING_END, CSS_HIDE ].join(SPACE) + '">', 0, '</div>'],
		...
		prototype: {
			...
			_syncMonthDays: function() {
				...
				instance.monthDays.each(
					function(node, index) {
						node.toggleClass(CSS_HIDE, (index >= daysInMonth));
			...
			_syncPaddingEnd: function() {
				...
				if (instance.get(SHOW_OTHER_MONTH)) {
					...
					instance.paddingDaysEnd.each(
						function(node, index) {
							node.toggleClass(CSS_HIDE, (index >= totalVisiblePaddingEnd));
			...
			_syncPaddingStart: function() {
				...
				paddingNodes.each(
					function(node, index) {
						...
						node.toggleClass(CSS_HIDE, (index < totalHidden));
